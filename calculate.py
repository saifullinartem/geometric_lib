...
function_mapping = {
    "circle": {"perimeter": circle.perimeter, "area": circle.area},
...
def calc(fig, func, size):
    ...
    if func not in funcs:
        raise ValueError(
            f"Function {func} is not supported. Available functions: {funcs}"
        )

    expected_size = sizes.get(f"{func}-{fig}", 1)
    if len(size) != expected_size:
        raise ValueError(
            f"Invalid number of sizes for {fig} {func}: expected {expected_size}, got {len(size)}"
        )
    ...
    if fig == "circle":
        ...
        if func == "perimeter":
            operation_str = (
                f"Perimeter of {fig} ({' + '.join(map(str, size))}) = {result}"
            )
        elif func == "area":
